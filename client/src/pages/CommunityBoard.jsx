...
import { useQuery, useMutation } from '@apollo/client';
import { GET_ALL_POSTS } from '../utils/queries';
import { CREATE_POST, ADD_COMMENT } from '../utils/mutations';  // <- Added ADD_COMMENT mutation import
import moment from 'moment';
import '../styles/CommunityBoard.css';
...
const CommunityBoard = () => {
  ...
  const [addComment] = useMutation(ADD_COMMENT, {  // <- New mutation hook for adding comment
    refetchQueries: [{ query: GET_ALL_POSTS }],
  });
  ...
  const handleCommentSubmit = async (postId) => {  // <- New function for submitting a comment
    if (newComment.trim() !== '') {
      try {
          ...
          <div className="post" key={post._id}>
            ...
            {openedPostId === post._id && (
              <div>
                ...
                {/* Added comment input section below */}
